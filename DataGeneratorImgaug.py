...
class DataManager(ImageDataGenerator):
    def __init__(self,
        ...
        self.imgaugment_vars                        = imgaugment_vars
...
class DataGenerator(Iterator):
    ...
    def next(self):
        ...
        fake_z_target = np.zeros((batch_data[self.parent.input_vars[0]].shape[0], 1))

        if self.parent.variational_model:
            ...
            return [batch_data[n] for n in self.parent.input_vars] + [maskbatch], [batch_target[n] for n in self.parent.output_vars]
    ...
    def _get_batches_of_transformed_samples(self, index_array):
        ...
        if self.category == 'train':
            # augmentation
            maskbatch_aug = np.zeros_like(maskbatch, dtype=np.bool)
            batch_data_aug = copy.deepcopy(batch_data)
            batch_target_aug = copy.deepcopy(batch_target)
            mask_segmap = ia.SegmentationMapsOnImage(self.parent.mask.astype(np.bool), shape = self.parent.mask.shape)
            for i in range(len(curr_indices)):
                seq_det = self.seq.to_deterministic()
                mask_augmented = seq_det.augment_segmentation_maps(mask_segmap)
                maskbatch_aug[i] = mask_augmented.get_arr().astype(np.bool)[:,:,np.newaxis]
                for name in self.parent.input_vars:
                    if name in self.parent.imgaugment_vars:
                        batch_data_aug[name][i] = seq_det.augment_image(batch_data[name][i])
                for name in self.parent.output_vars:
                    if name in self.parent.imgaugment_vars:
                        batch_target_aug[name][i] = seq_det.augment_image(batch_target[name][i])
        else:
            maskbatch_aug = copy.deepcopy(maskbatch)
            batch_data_aug = copy.deepcopy(batch_data)
            batch_target_aug = copy.deepcopy(batch_target)

        fake_z_target = np.zeros((batch_data_aug[self.parent.input_vars[0]].shape[0], 1))

        # return [batch_data_aug[n] for n in self.parent.input_vars] + [maskbatch_aug], [batch_target_aug[n] for n in self.parent.output_vars] + [fake_z_target]
        if self.parent.variational_model:
            return [batch_data_aug[n] for n in self.parent.input_vars] + [maskbatch_aug], [batch_target_aug[n] for n in self.parent.output_vars] + [fake_z_target]
        else:
            return [batch_data_aug[n] for n in self.parent.input_vars] + [maskbatch_aug], [batch_target_aug[n] for n in self.parent.output_vars]