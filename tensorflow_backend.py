...
from tensorflow.keras.backend import permute_dimensions, int_shape, dtype, floatx, image_data_format
...
def extract_image_patches(x, ksizes, ssizes, padding='same',
    ...
    patches = tf.compat.v1.extract_image_patches(x, kernel, strides, [1, 1, 1, 1],
                                       padding)
    # Reshaping to fit Theano
    bs, w, h, ch = int_shape(patches)
    patches = tf.reshape(tf.transpose(tf.reshape(patches, [-1, w, h, tf.compat.v1.floordiv(ch, ch_i), ch_i]), [0, 1, 2, 4, 3]),
...
def depth_to_space(input, scale, data_format=None):
    ...
    out = tf.compat.v1.depth_to_space(input, scale)
